...
@testset "`mt_coherence` reference test" begin
    ...
    sin_1 = sinpi.(2 * 12.0 * t)  # 12 Hz sinusoid signal
    sin_2 = sinpi.(2 * 12.0 * t .+ 1)
    noise = vec(reference_data("noise.txt", '\n')) # generated by `rand(1024) * 2 .- 1`
...
@testset "`mt_cross_power_spectra`" begin
    ...
    csd_array_multitaper_frequencies = reference_data("csd_array_multitaper_frequencies.txt", '\n')
    csd_array_multitaper_values_re = reshape(reference_data("csd_array_multitaper_values_re.txt", '\n'), (2,2,512))
    csd_array_multitaper_values_im = reshape(reference_data("csd_array_multitaper_values_im.txt", '\n'), (2,2,512))
