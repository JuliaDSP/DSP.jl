...
function Butterworth(::Type{T}, n::Integer) where {T<:Real}
    ...
    for i = 1:div(n, 2)
        ...
        @inbounds poles[2i-1] = pole
        @inbounds poles[2i] = conj(pole)
    end
    if isodd(n)
        poles[end] = -1
    end
...
function chebyshev_poles(::Type{T}, n::Integer, ε::Real) where {T<:Real}
    ...
    for i = 1:div(n, 2)
        ...
        @inbounds p[2i-1] = pole
        @inbounds p[2i] = conj(pole)
    end
    if isodd(n)
...
function Chebyshev2(::Type{T}, n::Integer, ripple::Real) where {T<:Real}
    ...
    for i = 1:div(n, 2)
        ...
        @inbounds z[2i-1] = ze
        @inbounds z[2i] = conj(ze)
        @inbounds k *= abs2(p[2i]) / abs2(ze)
    end
...
function landen(k::Real)
    ...
    # Eq. (50)
    for i = 1:niter
        @inbounds kn[i] = k = abs2(k / (1 + sqrt(1 - abs2(k))))
    end
    kn
end
    ...
    @eval begin
        function $fn(u::Number, landen::Vector{T}) where {T<:Real}
            ...
            for i = length(landen):-1:1
                oldwinv = winv
                @inbounds winv = 1 / (1 + landen[i]) * (winv + landen[i] / winv)
            end
            w = inv(winv)
        end
    end
...
function transform_prototype(ftype::Highpass, proto::ZeroPoleGain{:s})
    ...
    for i = 1:nz
        num *= -z[i]
        @inbounds newz[i] = ftype.w / z[i]
    end

    den = one(eltype(p))
    for i = 1:np
        @inbounds den *= -p[i]
        @inbounds newp[i] = ftype.w / p[i]
    end
...
function transform_prototype(ftype::Bandpass, proto::ZeroPoleGain{:s})
    ...
    for (oldc, newc) in ((p, newp), (z, newz))
        for i in eachindex(oldc)
            @inbounds b = oldc[i] * ((ftype.w2 - ftype.w1) / 2)
            pm = sqrt(muladd(-ftype.w2, ftype.w1, b^2))
            @inbounds newc[2i-1] = b + pm
            @inbounds newc[2i] = b - pm
        end
    end
...
function transform_prototype(ftype::Bandstop, proto::ZeroPoleGain{:s})
    ...
    for i = 1:nz
        @inbounds num *= -z[i]
        @inbounds b = (ftype.w2 - ftype.w1) / 2z[i]
        pm = sqrt(muladd(-ftype.w2, ftype.w1, b^2))
        @inbounds newz[2i-1] = b - pm
        @inbounds newz[2i] = b + pm
    end

    den = one(eltype(p))
    for i = 1:np
        @inbounds den *= -p[i]
        @inbounds b = (ftype.w2 - ftype.w1) / 2p[i]
        pm = sqrt(muladd(-ftype.w2, ftype.w1, b^2))
        @inbounds newp[2i-1] = b - pm
        @inbounds newp[2i] = b + pm
    end
    ...
    for (n, newc) in ((np, newp), (nz, newz))
        for i = n+1:npairs
            @inbounds newc[2i-1] = -npm
            @inbounds newc[2i] = npm
        end
    end
...
function bilinear(f::ZeroPoleGain{:s,Z,P,K}, fs::Real) where {Z,P,K}
    ...
    for i = 1:length(f.z)
        @inbounds z[i] = (2 + f.z[i] / fs) / (2 - f.z[i] / fs)
        @inbounds num *= (2 * fs - f.z[i])
    end

    den = one(one(fs) - one(P))
    for i = 1:length(f.p)
        @inbounds p[i] = (2 + f.p[i] / fs) / (2 - f.p[i] / fs)
        @inbounds den *= (2 * fs - f.p[i])
    end
...
function scalefactor(coefs::Vector, ::Highpass, fs::Real)
    ...
    for k in 1:length(coefs)
        @inbounds c += ifelse(isodd(k), coefs[k], -coefs[k])
    end
    c
end
function scalefactor(coefs::Vector, ftype::Bandpass, fs::Real)
    ...
    c = zero(coefs[1])
    for k = 0:n-1
        @inbounds c += coefs[k+1] * cospi(freq * (k - (n - 1) / 2))
    end
    c
end
